%% Simultaneous Fitting of Hair Conditioner Datasets
% Fit the 7 hair conditioner datasets
clear all; close all; clc;

%% Define EXACTLY the datasets you want
datasets = {
    %{'Experimental Data/6Khairconditioner_zspectra.mat', 'db19', 'offset', '1band', 13.8, '6K hc dB19'}; % wrong
    ...
    %{'Experimental Data/0716hc2.mat', 'hc19dbpos', 'offset', '1band', 13.8, '0716 hc 19dB pos'}; % wrong
    %{'Experimental Data/0716hc2.mat', 'hc19dbneg', 'offset', '1band', 13.8, '0716 hc 19dB neg'}; % wrong
    ...
    %{'Experimental Data/6Khairconditioner_zspectra.mat', 'db43', 'offset', '1band', 1.22, '6K hc dB43'}; % wrong
    %{'Experimental Data/10KHC_zpectra.mat', 'hc31dbsingle', 'offset', '1band', 3.45, '10K hc 31dB single'};
    %{'Experimental Data/0716hc.mat', 'hc22dbdual', 'offset', '2band', 9.76, '0716 hc 22dB dual'}; %correct

    {'Experimental Data/10KHC_zpectra.mat', 'hc28dbdual', 'offset', '2band', 4.88, '10K hc 28dB dual'}; %correct
    %{'Experimental Data/0716hc2.mat', 'hc16dbdualart', 'offsetart', '2band', 19.52, '0716 hc 16dB dual art'}; % wrong
...
param_names = {'R1\_free', 'R2\_free', 'M0\_semi', 'R1\_semi', 'R1D\_semi', 'f\_semi', 'T2\_semi', 'k'};
...
for i = 1:num_datasets
    ...
    for j = 1:n_params
        ...
        % Check if sim_opt equals sim_perturb (unidentifiable parameter)
        if isequal(sim_opt, sim_perturb)
            fprintf('  Parameter %d (%s): No effect detected, setting gradient to epsilon\n', j, param_names{j});
            J_dataset(:,j) = epsilon * ones(size(sim_opt));
        else
...
% Add text labels to existing correlation matrix
for i = 1:n_params
    for j = 1:n_params
        if pCorr(i,j) > 0.5
            col = 'w';
